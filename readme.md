...
- backs up the stock nginx config once, then writes the reverse-proxy config to
  ...
  - `GET /monitor` → serves `~/www/baby-monitor-notify.html` (using `$HOME` at run time,
    rather than a hardcoded path, so it keeps working if Termux's data directory ever changes)
